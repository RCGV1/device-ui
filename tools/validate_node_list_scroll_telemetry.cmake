...
foreach(IMPLEMENTATION legacy virtual_candidate)
    set(OUTPUT_PATH "${CMAKE_CURRENT_BINARY_DIR}/node-list-scroll-telemetry-${IMPLEMENTATION}.json")
    file(REMOVE "${OUTPUT_PATH}")
    execute_process(
        COMMAND "${NODE_LIST_BENCH}" --nodes 250 --trials 1 --seed 42 --implementation "${IMPLEMENTATION}" --json "${OUTPUT_PATH}"
        RESULT_VARIABLE RESULT
    )
    if(NOT RESULT EQUAL 0)
        message(FATAL_ERROR "node_list_bench failed for ${IMPLEMENTATION}: ${RESULT}")
    endif()
    ...
    if(ERROR OR NOT TELEMETRY_TYPE STREQUAL "OBJECT")
        message(FATAL_ERROR "${IMPLEMENTATION} is missing scroll_telemetry: ${ERROR}")
    endif()
    foreach(FIELD cycles rows_per_cycle sample_count frame_count elapsed_ns average_fps worst_frame_ns)
        ...
        if(ERROR OR NOT FIELD_TYPE STREQUAL "NUMBER")
            message(FATAL_ERROR "${IMPLEMENTATION} scroll_telemetry.${FIELD} must be numeric: ${ERROR}")
        endif()
    endforeach()
    ...
    if(NOT CYCLES EQUAL 2 OR NOT ROWS EQUAL 250 OR NOT SAMPLES EQUAL EXPECTED_SAMPLES OR NOT FRAMES EQUAL SAMPLES OR
       ELAPSED LESS_EQUAL 0 OR FPS LESS_EQUAL 0 OR WORST_FRAME LESS_EQUAL 0 OR WORST_FRAME GREATER ELAPSED)
        message(FATAL_ERROR "${IMPLEMENTATION} scroll telemetry is not a complete 250-row frame sample")
    endif()
    file(REMOVE "${OUTPUT_PATH}")
endforeach()
