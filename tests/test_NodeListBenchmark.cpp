...
TEST_CASE("benchmark CLI accepts the production compatibility option")
{
    const char *arguments[] = {"node_list_bench",
                               "--nodes",
                               "25",
                               ...
                               "production",
                               "--json",
                               "/tmp/node-list-production.json"};
    NodeListBenchmarkOptions options{};
    std::string jsonPath;

    REQUIRE(parseNodeListBenchmarkCommandLine(11, arguments, options, jsonPath));
    CHECK(jsonPath == "/tmp/node-list-production.json");
}
...
TEST_CASE("production benchmark reports allocator churn from real LVGL node-list syncs")
{
    NodeListBenchmarkOptions options{25, 1, 42, 0};
    const auto report = runNodeListBenchmark(options);

    CHECK(report.implementation.name == "production");
...
TEST_CASE("production JSON identifies virtual-list structural checks")
{
    const auto report = runNodeListBenchmark({25, 1, 42, 0});
    const auto outputPath = std::filesystem::temp_directory_path() /
                            ("device-ui-node-list-production-" + std::to_string(reinterpret_cast<uintptr_t>(&report)) + ".json");
    std::string error;
    ...
    CHECK(std::string(document["implementation"]["name"].GetString()) == "production");
...
TEST_CASE("benchmark JSON output is valid and contains the documented sections")
{
    ...
    CHECK(timing["virtual_refresh_ns"].IsObject());
    ...
    CHECK(correctness["candidate"].IsObject());
